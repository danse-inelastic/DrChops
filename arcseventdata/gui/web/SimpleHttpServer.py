...
"""
This script is downloaded from .....(link here).

It is not the most suitable server. We need a server that is even
simpler than this one. We almost certainly don't need the 'browse'
functionality. 

We need to limit access to local connections.
"""
...
class ScriptRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def run_py(self, script):
        ...
        namespace = {
           'request':self.body, 'headers' : self.headers,
           #'file_handle_for_posted_data': self.rfile,
