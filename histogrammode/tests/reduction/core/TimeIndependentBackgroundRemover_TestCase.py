...
class TimeBG_TestCase(unittest.TestCase):
    ...
    from reduction.core.TimeIndependentBackgroundRemover import TimeIndependentBackgroundRemover as Remover  #overload this to test different sublass
    ...
    def test(self):
        ...
        hist = histogram(
            'Idpt',
            [
            ('detectorID', range(100)),
            ('pixelID', range(40) ),
            ('tof', arange(2000, 4000, 100), 'microsecond' ),
            ] )
        hist[(),(),()] = signal, signal
        ...
        mask = DetectorMask()
        remover = self.Remover(tbgMin, tbgMax )
        remover( hist, mask )
