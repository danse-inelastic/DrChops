...
def findTKTestModules( pre ):
    ...
    for entry in os.listdir( curdir ):
        ...
        #if not entry.find("SimpleFitter") != -1: continue #this is here because simple fitter uses scipy. only if scipy is avaiable this should fail
...
def createsuites():
    modules = findTKTestModules("vectorCompat")
