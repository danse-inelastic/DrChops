;----------------------------------------------------------------------------
;----------------------------------------------------------------------------
;-- ROUTINE:    read_ldaq_data_r
;--
;-- USEAGE: read_data_r, str
;--
;--     read_data, str, filename='filename'
;--
...
;-- 11 June 2008; FT
;-- Changed to throw out 16 short detectors that are now at 40-55
;----------------------------------------------------------------------------
;----------------------------------------------------------------------------
...
PRO read_ldaq_r_1,str,filename=filename
    COMMON nexus,fhandle,sdhandle,vname,vtype,vval

    ;-------------------------------------------------------
    ;-- defining path for the dialog box, to pick files from
    ;--------------r-----------------------------------------
    datapath='/home/PharosData/'
    ...
    st = getobj("/NXentry:run_*/NXdata:monitors_tof/SDS:data",-1,-1)
    if (st ne 0) then mon=vval
    st = getobj("/NXentry:run_*/NXdata:monitors_tof/SDS:tof",-1,-1)
    if (st ne 0) then mon_time=vval
    ...
    st = getobj("/NXentry:run_*/NXdata:psd_y_vs_tof_grouped/SDS:data",-1,-1)
    if (st ne 0) then grouped=vval
    st = getobj("/NXentry:run_*/NXdata:psd_y_vs_tof_grouped/SDS:d_spacing",-1,-1)
    if (st ne 0) then grouped_time=vval
    ...
    size_grouped=SIZE(grouped,/DIMENSIONS)
    ...
    str={runno:0, title:'', tstart:'', tend:'', uamphrs:0.0,   $
       mon:FLTARR(SIZE(mon,/DIMENSIONS)),       $
       mon_time:FLTARR(SIZE(mon_time,/DIMENSIONS)),       $
       tof:FLTARR(size_tof[0],size_tof[1] - 16),        $
       tof_time:FLTARR(size_tof[0]+1),          $
       grouped:FLTARR(size_grouped[0],size_grouped[1]),        $
       grouped_time:FLTARR(size_grouped[0]+1),          $
    ...
    str.uamphrs=uamphrs
    str.mon=mon
    str.mon_time=mon_time/10.
    str.big_time=big_time/10.
    str.big_y=big_y
    str.tof_time=tof_time/10.
    str.grouped=grouped
    str.grouped_time=grouped_time
    str.pos_y=pos_y
    str.pos_y=pos_y
