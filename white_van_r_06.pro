	PRO WHITE_VAN_r_06,ei,phi,eff,ab,vfile=vfile,strv=strv
...
;
	IF (N_ELEMENTS(strv) EQ 0) THEN BEGIN
	   IF (N_ELEMENTS(vfile) EQ 0) THEN 				$
		vfile=DIALOG_PICKFILE(PATH='/home/pharos/unzipped',	$
		TITLE='Pick a white bean vanadium file')
;	   read_data,strv,file=vfile
; to bypass increased number of detectors
	   read_data_r,strv,file=vfile
...
;	eff[112:375]=strv.tof[iarr,112:375]/ab[112:375]/eff10[0]
	eff[0:263]=TOTAL(strv.tof[iarr-50:iarr+50,0:263],1)/ab[0:263]/eff10[0]
	eff[264:375]=TOTAL(strv.tof[iarr-50:iarr+50,264:375],1)/ab[264:375]/eff6[0]
	eff=eff/TOTAL(eff)*376
; these are new lines provided by Frans to fix problem with new detectors number
	index=WHERE(eff EQ 0, count)
    	IF (count NE -1) THEN eff(index)=1.
; these are old lines 
;	index=WHERE(eff EQ 0)
;	IF (index NE -1) THEN eff(WHERE(eff EQ 0))=1.
;
	END
